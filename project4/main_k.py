...
def run_kalman_nca(x, y, q=0.1, r=1.0):
    ...
    state[0,0] = x[0]
    state[1,0] = y[0]
    P = np.diag([10,10,100,100,1,1]).astype(np.float32)
    ...
    for xi, yi in zip(x[1:], y[1:]):
        ...
        sx.append(float(state[0, 0]))
        sy.append(float(state[1, 0]))

    return np.array(sx), np.array(sy)

class ParticleFilter():
    def __init__(self, n_particles=100, alpha=1e-3, q_scale=.01):
    ...
    def name(self):
        return f"ParticleFilter_200_0.9"
